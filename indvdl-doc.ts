 ...
 *
 * 끝의 '>' 가 중요하다. 문서에 따라 "마. 상위 5명의 개인별 보수현황" 같은
 * 목차성 제목이 표보다 앞에 따로 있어서, 그걸 잡으면 구간이 어긋난다.
 * 표 제목은 항상 <보수지급금액 … 개인별 보수현황> 꼴이라 '>' 로 구분된다.
 */
 ...
 */
function sliceSections(doc: string): string[] {
  const slices: string[] = [];
  let from = 0;

  while (true) {
    const at = doc.indexOf(SECTION_MARKER, from);
    if (at === -1) break;
    from = at + SECTION_MARKER.length;

    // 제목 다음의 LIBRARY 들이 사람별 표다.
    const start = doc.indexOf("<LIBRARY", from);
    if (start === -1) continue;

    // 다음 표 제목이 나오면 거기서 끊는다. 반드시 start 이후에서 찾아야 한다.
    const nextTitle = doc.indexOf("<보수지급금액", start);
    const end = nextTitle === -1 ? doc.length : nextTitle;
    if (end <= start) continue;

    slices.push(doc.slice(start, end));
  }
  return slices;
}
...
export function parseIndvdlDoc(doc: string): IndvdlDocRow[] {
  // 제목이 여러 번 나오는 문서가 있어(정정 공시, 목차 등) 결과가 나올 때까지 시도한다.
  for (const section of sliceSections(doc)) {
    const rows = parseSection(section);
    if (rows.length) return rows;
  }
  return [];
}

function parseSection(section: string): IndvdlDocRow[] {
